...
make.simmap<-function(tree,x,model="SYM",nsim=1,...){
	...
	if(inherits(tree,"multiPhylo")){
		...
		if(is.character(Q)&&Q=="empirical"){
			...
			print("Mserrano = using the asr-bisse anc.lik in L object, but only when Q empirical")
			L<-XX$L
			Q<-XX$Q
			print(Q)
			logL<-XX$loglik
			...
			print("Mserrano = using the asr-bisse anc.lik in L object, but only when Q fixed")
			L<-XX$L
			logL<-XX$loglik
			if(pi[1]=="equal") pi<-setNames(rep(1/m,m),colnames(L)) # set equal
			else if(pi[1]=="estimated") pi<-statdist(Q) # set from stationary distribution
...
# get pars
# written by Liam J. Revell 2013
## modified by M.SErrano to include the asr-bisse in the obj$lik.anc after called L 

getPars<-function(bt,xx,model,Q,tree,tol,m,liks=TRUE,pi){
	print("getPars function")
	resBisse<-estimatingBisse(tree, xx)
	Bisse_asr<- resBisse[1]
	Q <- resBisse[2][[1]]
	print(Q)	
#	obj<-fitMk(bt,xx,model,fixedQ=Q,output.liks=liks,pi=pi)
	obj<-fitMk(bt,xx,model,fixedQ=NULL,output.liks=liks,pi=pi)
	#plot(tree, show.tip.label =FALSE)
	#nodelabels(thermo=t(Bisse_asr[[1]]), piecol=1:2, cex=.5)
	new_anc<- t(Bisse_asr[[1]])
	rownames(new_anc)<-rownames(obj$lik.anc)
	obj$lik.anc<-new_anc
	print("MSerrano = Using the anc from asr-bisse while getting getPars fx")
...
## Edited by MSerrano to include the asr-BiSSE
## warning change the path to the consensus only for bisse estimation once
## it works with the map_migrations_ script as it uses arguments from it

estimatingBisse <- function(tree, x) { 
	mtest<- match.phylo.data(tree, x[,2])
	parsBisse<-starting.point.bisse(mtest$phy)
	likBisse <- make.bisse(mtest$phy, mtest$data, sampling.f=c(0.48, 0.53))
	fitBisse <- find.mle(likBisse, parsBisse, method="subplex")
	stBisse <- asr.marginal(likBisse, coef(fitBisse))
	Q=matrix(ncol=2, nrow=2)
	Q[1,2]<-coef(fitBisse)[5]
	Q[1,1]<-(-coef(fitBisse)[5])
	Q[2,1]<-coef(fitBisse)[6]
	Q[2,2]<-(-coef(fitBisse)[6])
	colnames(Q)<-c("0","1")
	rownames(Q)<-c("0","1")
print("estimating Bisse done!")
 return(list(stBisse, Q))
} 
